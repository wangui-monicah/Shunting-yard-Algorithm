...
public class MyTreeNode<T extends Comparable<T>> {
	...
	public String strDataInOrder(String s) {
		s += this.data +",";
		if(this.leftChild != null) 
			s = this.leftChild.strDataInOrder(s);
		if(this.rightChild != null) 
			s = this.rightChild.strDataInOrder(s);
		return s;
	}
	public String strStructInOrder(String s) {
		s += 1 + "";
		if(this.leftChild != null) 
			s += this.leftChild.strStructInOrder("");
		else
			s += 0;
		if(this.rightChild != null) 
			s += this.rightChild.strStructInOrder("");
		else
			s += 0;
		return s;
	}
